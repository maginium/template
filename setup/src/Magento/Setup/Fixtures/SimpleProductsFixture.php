...
class SimpleProductsFixture extends Fixture
{
    ...
    public function execute()
    {
        ...
        $fixtureMap = [
            'name' => fn($productId) => sprintf('Simple Product %s', $productId),
            'sku' => fn($productId) => sprintf($this->getSkuPattern(), $productId),
            'price' => fn($index, $entityNumber) => $this->priceProvider->getPrice($entityNumber),
            'url_key' => fn($productId) => sprintf('simple-product-%s', $productId),
            'description' => fn($index) => $descriptionGenerator->generate($index),
            'short_description' => fn($index) => $shortDescriptionGenerator->generate($index),
            'website_ids' => fn($index, $entityNumber) => $this->websiteCategoryProvider->getWebsiteIds($index),
            'category_ids' => fn($index, $entityNumber) => $this->websiteCategoryProvider->getCategoryId($index),
            'meta_keyword' => fn($productId) => sprintf($this->getSkuPattern(), $productId),
            'meta_title' => fn($productId) => sprintf($this->getSkuPattern(), $productId),
            'attribute_set_id' => $attributeSet,
            'additional_attributes' => $additionalAttributes,
            'status' => fn() => Status::STATUS_ENABLED,
        ];
    ...
    {
        if ($this->descriptionConfig === null) {
    ...
    {
        if ($this->shortDescriptionConfig === null) {
    ...
    {
        if ($this->additionalAttributeSetIds === null) {
