...
class Level23ProcessingDataBuilder implements BuilderInterface
{
    ...
    public function build(array $buildSubject): array
    {
        ...
        $includeTax = $this->taxHelper->priceIncludesTax($order->getStoreId());
        ...
        if (($ppSendCartLineItems && $isPayPalMethod) || ($btSendLineItems && !$isPayPalMethod)) {
            foreach ($order->getItems() as $item) {
                ...
                $itemUnitPrice = (float)$item->getBasePrice();
                ...
                $lineItems[] = array_combine(
                    ...
                    [
                        ...
                        $this->numberToString((float)$item->getQtyOrdered() * $itemUnitPrice, 2),
        ...
        if ($order->getShippingAddress()) {
            ...
            $processingData[self::KEY_SHIPPING_AMT] = $this->numberToString(
                $includeTax ? $order->getBaseShippingInclTax() : $order->getBaseShippingAmount(),
                2
            );
