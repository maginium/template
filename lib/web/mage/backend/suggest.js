...
define(["jquery", "mage/template", "mage/mage", "jquery/ui", "mage/backend/menu", "mage/translate"], function ($, mageTemplate) {
  "use strict";
  ...
  $.widget("mage.suggest", {
    ...
    _render: function () {
      ...
      wrapper = this.options.className ? $(this.options.inputWrapper).addClass(this.options.className) : $(this.options.inputWrapper);
      this.element.wrap(wrapper)[this.options.appendMethod](this.dropdown).attr("autocomplete", "off");
    },
    ...
    _prepareValueField: function () {
      if (this.options.valueField) {
        ...
        this.valueField = this._createValueField().insertBefore(this.element).attr("name", this.element.attr("name"));
    ...
    _focusItem: function (e, ui) {
      if (ui && ui.item) {
        this._focused = $(ui.item).prop("tagName") ? this._readItemData(ui.item) : ui.item;
    ...
    _prepareDropdownContext: function (context) {
      return $.extend(context, {
        ...
        optionData: function (item) {
          return 'data-suggest-option="' + $("<div>").text(JSON.stringify(item)).html().replace(/"/g, "&quot;") + '"';
        },
  ...
  $.widget("mage.suggest", $.mage.suggest, {
    ...
    _getOption: function (item) {
      return $(item).prop("tagName") ? $(item) : this.valueField.find("option[value=" + item.id + "]");
    },
