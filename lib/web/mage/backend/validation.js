...
define(["jquery", "underscore", "Magento_Ui/js/modal/alert", "jquery/ui", "jquery/validate", "mage/translate", "mage/validation"], function (
  $,
  _,
  alert,
) {
  ...
  _.each(
    {
      "validate-greater-zero-based-on-option": [
        function (v, el) {
          var optionType = $(el).closest(".form-list").prev(".fieldset-alt").find("select.select-product-option-type"),
        ...
        function (v, element) {
          ...
          if (linkType.is(":checked") && (v === "" || v === "[]")) {
            ...
            if (!alertAlreadyDisplayed && (newFileContainer.empty() || newFileContainer.is(":visible"))) {
              ...
              alert({
                content: $.mage.__("There are files that were selected but not uploaded yet. " + "Please upload or remove them first"),
              });
            }
