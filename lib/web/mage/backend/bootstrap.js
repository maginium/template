...
/* global FORM_KEY */
define(["jquery", "mage/apply/main", "mage/backend/notification", "Magento_Ui/js/lib/knockout/bootstrap", "mage/mage", "mage/translate"], function (
  $,
  mage,
  notification,
) {
  ...
  $.ajaxSetup({
    ...
    beforeSend: function (jqXHR, settings) {
      ...
      if (!settings.url.match(new RegExp("[?&]isAjax=true", ""))) {
        settings.url = settings.url.match(new RegExp("\\?", "g")) ? settings.url + "&isAjax=true" : settings.url + "?isAjax=true";
      }
      ...
      } else if (typeof settings.data === "string" && settings.data.indexOf("form_key=") === -1) {
        settings.data +=
          "&" +
          $.param({
            form_key: formKey,
