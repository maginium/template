    ...
    var self = null,
      magentovariable = function (editor) {
          ...
          decodeVariables: function (content) {
            ...
            [].forEach.call(doc.querySelectorAll("span.magento-variable"), function (el) {
              ...
              if ($el.hasClass("magento-custom-var")) {
                $el.replaceWith(customDirectiveGenerator.processConfig(Base64.idDecode($el.attr("id"))));
              } else {
                $el.replaceWith(configDirectiveGenerator.processConfig(Base64.idDecode($el.attr("id"))));
              }
            });
        ...
        editor.addCommand("openVariablesSlideout", function (commandConfig) {
          ...
          MagentovariablePlugin.loadChooser(config.url, wysiwyg.getId(), selectedElement);
        });
