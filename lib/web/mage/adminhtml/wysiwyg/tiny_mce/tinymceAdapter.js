...
define([
  ...
  tinyMceWysiwyg.prototype = {
    ...
    initialize: function (htmlId, config) {
      ...
      _.bindAll(this, "beforeSetContent", "saveContent", "onChangeContent", "openFileBrowser", "updateTextArea", "onUndo", "removeEvents");
        ...
        if (controlId.tooltip) {
          ...
          if (enabled) {
            ...
            jQuery(controlId.getEl())
              .children("button")
              .addBack()
              .attr("style", "color: inherit;" + "background-color: inherit;" + "border-color: transparent;");
          }
        }
      });
    },
    ...
    openFileBrowser: function (o) {
      var typeTitle = this.translate("Select Images"),
        ...
        self = this,
        wUrl = this.config["files_browser_window_url"] + "target_element_id/" + this.getId() + "/" + "store/" + storeId + "/";
    ...
    makeDirectiveUrl: function (directive) {
      return this.config["directives_url"].replace(/directive/, "directive/___directive/" + directive).replace(/\/$/, "");
    },
    ...
    decodeDirectives: function (content) {
      var directiveUrl = this.makeDirectiveUrl("%directive%").split("?")[0], // remove query string from directive
        ...
        regexDirectiveUrl = regexEscapedDirectiveUrl.replace("%directive%", '([a-zA-Z0-9,_-]+(?:%2[A-Z]|)+\/?)(?:(?!").)*') + '/?(\\\\?[^"]*)?', // allow optional query string
