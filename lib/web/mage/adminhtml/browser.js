...
define([
  ...
  window.MediabrowserUtility = {
    ...
    setTargetElementId: function (options, url) {
      this.targetElementId = options && options.targetElementId ? options.targetElementId : url.match(/\/target_element_id\/([\s\S].*?)\//)[1];
    },
  ...
  $.widget("mage.mediabrowser", {
    ...
    _selectNode: function (event, data) {
      ...
      this.element.find("#delete_folder").toggleClass(this.options.hidden, node.id === "root"); //eslint-disable-line eqeqeq
      this.element.find("#content_header_text").html(node.id === "root" ? this.headerText : node.text); //eslint-disable-line eqeqeq
      ...
      return $.ajax({
        ...
        $.proxy(function (data) {
          if (typeof targetEl === "function") {
            ...
            targetEl.val(data).data("size", fileRow.data("size")).data("mime-type", fileRow.data("mime-type"));
          }
          MediabrowserUtility.closeDialog();
            ...
            $.ajax({
              ...
              $.proxy(function (data) {
                if (data.error) {
                  ...
                  self.tree.jstree("refresh_node", self.element.find('[data-id="' + self.activeNode.id + '"]'));
                }
              }, this),
            );
