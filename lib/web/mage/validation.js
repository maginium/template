...
define(["jquery", "moment", "mageUtils", "jquery-ui-modules/widget", "jquery/validate", "mage/translate"], function ($, moment, utils) {
  ...
  rules = {
      ...
      function (v, elm) {
        ...
        if (result === false) {
          validator.passwordErrorMessage = $.mage
            .__("Minimum length of this field must be equal or greater than %1 symbols. Leading and trailing spaces will be ignored.")
    ...
    "validate-currency-dollar": [
      function (v) {
        return (
          $.mage.isEmptyNoTrim(v) ||
          /^\$?\-?([1-9]{1}[0-9]{0,2}(\,[0-9]{3})*(\.[0-9]{0,2})?|[1-9]{1}\d*(\.[0-9]{0,2})?|0(\.[0-9]{0,2})?|(\.[0-9]{1,2})?)$/.test(v)
        ); //eslint-disable-line max-len
      },
  ...
  $.widget("mage.validation", {
    ...
    _create: function () {
      ...
      this.element.find(".field.additional").find(".control").find("input, select, textarea").removeAttr("aria-required");
