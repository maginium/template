...
class processor
{
    ...
    public function getViewFileUrl()
    {
        ...
        $errorPath = str_starts_with($errorDir, $indexDir) ?
    ...
    public function saveReport(array $reportData): string
    {
        ...
        if (isset($reportData['skin']) && $reportData['skin'] !== self::DEFAULT_SKIN) {
     ...
     * Get path to reports.
     *
     * @param int $reportDirNestingLevel
     * @param string $reportId
     *
     * @return string
     */
    ...
    private function getReportDirNestingLevel(string $reportId): int
    {
        ...
        if ($value === false && property_exists($this->_config, 'dir_nesting_level')) {
            $value = $this->_config->dir_nesting_level;
        }
        $value = (int)$value;
        $maxValue = $this->getMaxReportDirNestingLevel($reportId);

        return $value > 0 && $maxValue >= $value ? $value : 0;
    }
    ...
    {
        return (int)floor(mb_strlen($reportId) / self::NUMBER_SYMBOLS_IN_SUBDIR_NAME);
    }
}
